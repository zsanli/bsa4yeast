...
def rmUpaired(Args):
	...
	total_reads2 = 0
	while True:
		readid = infile1.readline().rstrip().split(' ')[0][0:-2]
		if len(readid) == 0:
			break
	...
	while True:
		readid = infile2.readline().rstrip().split(' ')[0][0:-2]
		if len(readid) == 0:
			break
...
#			print readid
#			print readid_1
			readid_new=readid_1+'/1 '
			#+readid.split(' ')[1]+' '+readid.split(' ')[2]
		#	print str(n)+readid+'\n'
			outfile1.write('%s\n%s\n%s\n%s\n' %(readid_new,seq,readid_new,qual))
	#print 'second start'
	n=0
	while True:
